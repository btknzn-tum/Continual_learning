...
\label{sec:classil}
All results above use task-aware evaluation. Merging all heads and taking a
global argmax (no task identity at test) gives a first class-IL picture
(seed 42, C100 10-task, layer3+4; 5-seed runs in progress): MAS-adapter
62.9 with only 2.6 forgetting, naive 61.3 with 32.1 forgetting, NCM
61.9--63.4, RanPAC 69.1 (layer4). Two observations survive the protocol
change: protection still converts catastrophic forgetting into near-zero
forgetting (the MAS--naive gap is now almost entirely \emph{cross-task
confusion}, not forgetting), and RanPAC's classifier-capacity advantage grows
in its native task-agnostic setting. On Split CIFAR-10 the multi-tap RanPAC
reaches 92.4 class-IL. \todo{complete 5-seed class-IL cells + placement
deltas under class-IL}
