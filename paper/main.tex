...
(supervised ResNet-50, CLIP ViT-B/32), five continual-learning streams
...
evaluation shows the placement effect persists and somewhat strengthens under
the harder protocol ($+2.6$ for RanPAC vs.\ $+1.0$ task-incremental). Finally,
we test whether minimal encoder plasticity (unfreezing the least-important
5\% of parameters) can raise the frozen-feature ceiling and report a careful
null result: against a matched frozen control it does not, on any stream ---
sharpening just how hard that ceiling is to cross.
\end{abstract}
...
\citep{ewclora2026,onlinelora2025}. All of these works, however,
...
\item \textbf{A careful null result on encoder plasticity.} We test whether
  unfreezing the least-important 5\% of encoder parameters (protected and
  anchored) beats frozen features, and show against a \emph{matched} control
  that it does not on any stream --- a result we report plainly because it
  quantifies how strong the frozen-feature ceiling is and where adaptation
  must improve (\cref{sec:beyond}).
\end{itemize}
...
Closest to us, \citet{ewclora2026} regularize a shared LoRA update with EWC and
\citet{onlinelora2025} apply importance-weighted low-rank adaptation online ---
both attach the protected module to the final representation. Our contribution
is orthogonal: the \emph{placement axis}, plus capacity-matched controls
neither reports.
...
ridge classifier; NCM \citep{rebuffi2017icarl,zhou2023revisiting} is the
prototype baseline. Most directly related, LayUP \citep{layup2024} concatenates
the last $k$ ViT blocks for \emph{prototype/Gram} classifiers and already
observes that intermediate representations are more robust to domain shift.
Our study differs on three axes: (i) we study \emph{gradient-trained protected
adapters}, not only closed-form prototype heads; (ii) we compare a supervised
CNN against a contrastive ViT and find the placement optimum \emph{flips}
between them; and (iii) we show the tap choice transfers to and improves RanPAC
itself, with capacity-matched controls. We thus generalize the ``read between
the layers'' insight from one classifier family to a placement principle that
holds across method families and encoders.
...
multi-tap MLP), matching multi-tap RanPAC (93.12$\pm$0.13, a statistical tie).
Two caveats
...
\textbf{The honest answer, against a matched control, is no.} It is tempting to
compare the plastic run (81.2 on the 5-dataset stream, seed 42, 10 epochs) to
the cached frozen benchmark (78.1) and claim a $+3.1$ gain --- but those
pipelines differ in featurization (per-task vs.\ global z-statistics) and
exposure. The correct control is the \emph{same} end-to-end pipeline with the
encoder frozen ($q{=}0$): it scores \textbf{82.0} (forgetting 2.9), i.e.,
higher than the plastic run and with less forgetting. Selective plasticity does
increase per-task learning --- diagonal accuracies rise (SVHN 63.7$\to$65.5,
KMNIST 80.7$\to$81.5) --- but the extra drift costs more in retention
(end-of-stream MNIST 95.9$\to$93.5) than it buys, netting a small \emph{loss}.
The same holds on C100 (10-task 89.1 vs.\ 90.2 frozen; 20-task 93.4 vs.\ 94.5).
At the settings we explored, unfreezing 5\% of the encoder does not beat frozen
features on any stream.

The ablations explain why and bound the negative result. Plasticity has a
\emph{budget}: at 20 epochs it over-drifts (77.0); quadrupling the trainable
share to $q{=}0.2$ trades accuracy for $2.6\times$ forgetting (81.9 at F=11.7).
The similarity anchor is a pure stability regularizer --- removing it
($\lambda{=}0$) leaves accuracy unchanged (81.9) but raises forgetting
4.5$\to$7.3. In short, every knob that adds plasticity adds net forgetting
faster than accuracy, which is precisely why the frozen control wins. We
report this null result plainly: it reinforces the paper's overarching finding
that on these benchmarks frozen features set a ceiling that is very hard to
cross, and it delimits where future encoder-adaptation work must improve (the
stability side) to pay off. A combination with a stronger classifier head is
examined next.

\paragraph{Combining with a RanPAC head.} Since the classifier-capacity gap
(\cref{sec:honest}) rather than the encoder limits our adapter, we also fit a
streaming RanPAC ridge head on the \emph{same} lightly adapted encoder's
multi-tap features. On the adapted encoder the ridge head scores 86.9 (task-IL)
/ 86.7 (class-IL) on the 5-dataset stream --- reproducing the $+4.5$
classifier-capacity gap over the gradient MLP adapter (82.4) yet again, but
still \emph{below} the same head on the \emph{frozen} encoder (87.3 / 87.1).
The two independent classifiers thus agree: the small encoder adaptation we
apply does not improve the features in a way either head can exploit; the
frozen ceiling stands under both. Pushing plasticity harder is actively
destructive --- the aggressive $q{=}0.2$ variant corrupts the representation so
badly that even the ridge head collapses (70.2, from 86.9 at $q{=}0.05$). We read this consistency as evidence that
crossing it will require adaptation at a larger scale (more parameters or more
exposure) than a forgetting-safe budget allows here --- an honest boundary for
future work rather than a solved problem.
...
\textbf{the placement finding survives the harder protocol and somewhat
strengthens}: giving RanPAC our multi-tap features raises its class-IL accuracy
by $+2.6$ points on C100 (74.1\,$\to$\,76.7, 5 seeds), versus $+1.0$ under
task-IL --- where-you-tap matters at least as much when the evaluation is
harder. RanPAC dominates class-IL outright (76.7 vs.\ the adapter's 37.1),
...
Regularized adapters suppress forgetting but not cross-task confusion; RanPAC
dominates, and our multi-tap placement helps it at least as much as under
task-IL ($+2.6$ vs.\ $+1.0$ on C100, 5 seeds). $^*$single seed.}
\label{tab:classil}
...
\multirow{2}{*}{C100 (10 t.)}
 & last tap & 31.9$\pm$0.8 (58.3) & 34.3$\pm$1.3 (4.8) & 63.4 & 74.1$\pm$0.2 \\
 & multi-tap & 29.6$\pm$2.2 (63.5) & 37.1$\pm$1.8 (6.5) & 61.9 & \best{76.7$\pm$0.1} \\
\midrule
CIFAR-10 & multi-tap & 62.6$\pm$3.4 (31.2) & 61.7$\pm$2.0 (1.5) & --- & \best{92.4$\pm$0.1} \\
5-Datasets & multi-tap & 26.6$\pm$3.1 (63.4) & 41.5$\pm$3.0 (10.3) & --- & \best{87.1$\pm$0.1} \\
\bottomrule
\end{tabular}
\end{table}
...
the deeper the useful taps. Attempting to go further, we find the frozen-feature
ceiling remarkably hard to cross: a forgetting-safe budget of encoder plasticity
does not beat a matched frozen control under either a gradient or a
random-projection classifier --- a boundary we report honestly to mark where
future adaptation work must improve. We release the full benchmark --- one
trainer, seven methods, two encoders, five streams, five seeds --- so that
future adapter-based CL methods can report \emph{where} they attach, not only
