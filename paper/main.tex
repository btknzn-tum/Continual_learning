...
Tap & Naive & MAS & EWC \\
\midrule
block3 & 42.0$\pm$1.2 (35.5) & 67.8$\pm$0.4 (1.9) & 57.9$\pm$2.4 (15.9) \\
block9 & \todo{} & 87.3$\pm$0.5 (1.0) & \todo{} \\
final & 91.6$^*$ (5.8) & 94.1$^*$ (0.4) & \best{95.4$^*$} (1.0) \\
block9+final & 87.5$^*$ (10.2) & 93.9$^*$ (1.0) & 94.8$^*$ (1.6) \\
block3+final & --- & 92.8$^*$ (0.8) & --- \\
\bottomrule
\end{tabular}
\end{table}

\todo{Fill layer4/l3+4 joint cells; complete CLIP 5-seed numbers from running
grid; add Fig.~1 depth-vs-forgetting curves}

\subsection{Placement transfers to RanPAC}
\label{sec:transfer}
\begin{table}[t]
\centering\small
\caption{Applying our multi-tap placement to RanPAC (ResNet-50) improves it on
every stream --- a placement-only change to a published method. 5 seeds except
$^*$.}
\label{tab:transfer}
\begin{tabular}{lccc}
\toprule
Stream & RanPAC (layer4) & RanPAC (layer3+4) & $\Delta$ \\
\midrule
Split CIFAR-10 & 98.30$^*$ & 98.42$^*$ & +0.12 \\
Split CIFAR-100 (20 t.) & 95.88$\pm$0.10 & \best{96.15$\pm$0.26} & +0.27 \\
5-Datasets & 82.69$\pm$0.04 & \best{87.28$\pm$0.10} & \best{+4.59} \\
\bottomrule
\end{tabular}
\end{table}
...
NCM & 93.6 & 69.4 \\
RanPAC (+our placement) & \best{96.2$\pm$0.3} & \best{87.3$\pm$0.1} \\
\midrule
CLIP-final MAS & \todo{} & 82.8$\pm$0.5$^{n2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\label{sec:capacity}
\todo{h354 done (98.22/98.35 C10); h1024 + LoRA + C100 running in compare
chain; fill table T5}
...
\label{sec:limits}
Task-incremental protocol only (class-IL extension \todo{pending decision});
two encoders; classic small-image streams under an ImageNet/CLIP backbone;
placement searched over stage-level taps, not arbitrary layers.
\todo{expand after final results}
